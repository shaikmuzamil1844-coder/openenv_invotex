...
#
# Example:
#   bash scripts/validate-submission.sh https://muzamil1844-openenv-invotex.hf.space .
#
...
portable_mktemp() { mktemp "${TMPDIR:-/tmp}/validate-XXXXXX" 2>/dev/null || mktemp; }
run_with_timeout() {
  local secs="$1"; shift
...
if [ -z "$PING_URL" ]; then
  ...
  printf "  ping_url  Your HuggingFace Space URL (e.g. https://muzamil1844-openenv-invotex.hf.space)\n"
  exit 1
fi
...
if [ "$HTTP_CODE" = "200" ]; then
  ...
  fail "HF Space not reachable (connection timeout)"
  hint "Check Space is running: $PING_URL"
  stop_at "Step 1"
else
  fail "HF Space /reset returned HTTP $HTTP_CODE (expected 200)"
  hint "Try opening $PING_URL in browser."
  stop_at "Step 1"
fi
...
log "  Using Dockerfile from: $DOCKER_CONTEXT"
BUILD_OK=false
